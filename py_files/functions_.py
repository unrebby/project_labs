...
class PositionalEncoding(nn.Module):
    def __init__(self, emb_size, dropout, maxlen: int = 5000):
        super(PositionalEncoding, self).__init__()
        self.emb_size = emb_size
        self.dropout = nn.Dropout(dropout)
        self.maxlen = maxlen
        self.pos_embedding = self.generate_positional_encoding()

    def generate_positional_encoding(self):
        positions = torch.arange(self.maxlen).reshape(self.maxlen, 1)
